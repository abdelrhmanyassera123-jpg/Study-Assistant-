...
{"time_columns":[],"days":[],"groups":[],"group_label":"","note":""}

- "time_columns": لو الجدول فيه شريط أعمدة وقت فوقه (سواء منتظم كل نص ساعة، أو أعمدة كل واحدة معاد محاضرة كامل، أو أي شكل تاني)، اقرا عناوينها **كل عمود لوحده، عمود عمود من الشمال لليمين، من غير ما تقفز أو تخمّن**، وارجعها زي ما هي مكتوبة بالظبط — رقم كان العنوان ولا كلمة زي "استراحة/Break" أو "توافر الدكتور/Lecturer availability".
  **ممنوع مهم**: لو شفت نمط بيتكرر في أول الشريط (زي: 3 أعمدة وقت وبعدين عمود استراحة)، **ده مبررش تفترض إن نفس النمط هيفضل يتكرر بانتظام طول الشريط كله**. لازم تقرا كل جزء من الشريط فعليًا لوحده — ممكن عدد الأعمدة بين كل استراحة والتانية يختلف، أو الاستراحة تتكرر مرة واحدة بس مش عدة مرات. الغلط اللي بيحصل كتير: تخترع تكرار منتظم لعمود "استراحة" أو "توافر الدكتور" مكان ما هو موجود فعلاً مرة واحدة بس.
  لو فيه **فجوة وقت بين عمودين** (عمود بيخلص 12:30 واللي بعده بيبدأ 12:45 مثلاً، ربع ساعة مالهاش عمود خاص بيها ولا مكتوبة في أي مكان)، رجّع الأعمدة زي ما هي من غير ما تحاول تملا الفجوة أو تخترع عمود ليها.
  لو الجدول مفهوش شريط وقت ثابت أصلاً (كل محاضرة وقتها مكتوب جنبها مباشرة)، سيب "time_columns" فاضية.
  في الآخر عدّ الأعمدة اللي رجعتها وقارنها بعدد الخطوط الرأسية الفاصلة بينهم في صف العناوين نفسه — لو مش متطابقين ارجع صحّح قبل ما ترجّع الرد.
- "days": كل الأيام اللي فيها محاضرات في الجدول، بالترتيب اللي ظاهرة بيه، كل يوم بالعربي كامل (السبت، الأحد، الاثنين، الثلاثاء، الأربعاء، الخميس، الجمعة). ده هيتستخدم بعد كده عشان نستخرج محاضرات كل يوم لوحده.
...
''';
  ...
  static String scheduleEntriesSystem({
    ...
    final dayLine = day == null
        ? ''
        : '\n**دلوقتي مطلوب منك يوم "$day" بس** — استخرج محاضراته هو، '
            'وتجاهل تمامًا أي يوم تاني ظاهر في نفس الجدول حتى لو قدامك. '
            'أي محاضرة يومها مش "$day" بالظبط، سيبها ومترجعهاش.\n';
...
''';
  ...
  static String scheduleEntriesPrompt(String text, {String? day}) {
    final task = 'التركيب (الأعمدة والأقسام) معروف ومؤكد بالفعل زي ما اتقالك. '
        '${day == null ? 'دلوقتي استخرج كل محاضرة بالتفصيل.' : 'دلوقتي استخرج محاضرات يوم "$day" بس بالتفصيل، وسيب أي يوم تاني.'}';
    if (text.trim().isEmpty) {
